...
def check_watch(bundle_cache, watch):
    ...
    # "new showtimes" mode: date=="any" with a baseline snapshot -> only NEW dates
    seen = set(watch.get("seenDates") or [])
    ...
    if chain == "vox":
        ...
        if date_set:
            ...
            if seen:  # new-showtimes mode: drop dates already seen at watch start
                sess = [s for s in sess if s["displayDate"] not in seen]
    ...
    if date_set:
        ...
        target_days = [d for d in open_days if d not in seen]  # only new days
        if not target_days:
            return []
    else:
